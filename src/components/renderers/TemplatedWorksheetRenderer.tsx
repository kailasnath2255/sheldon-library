...
import type { TemplatedWorksheet } from "@/lib/template-types";
import { renderWorksheet } from "@/lib/template-engine";

export default function TemplatedWorksheetRenderer({ data }: { data: TemplatedWorksheet }) {
  ...
  useEffect(() => {
    ...
    renderWorksheet(data)
      ...
      .catch((e) => {
        if (!cancelled) { toast.error("Could not render worksheet: " + (e?.message || String(e))); setBusy(false); }
      });
  ...
  return (
    <div ref={wrapRef} className="bg-white dark:bg-deep-surface rounded-3xl ss-edge overflow-hidden flex flex-col h-[85vh]">
      <div className="flex items-center justify-between gap-2 px-4 py-3 border-b-2 border-ss-ink-900 dark:border-white/50 bg-soft-cream dark:bg-deep-cream/40">
        <span className="eyebrow">Worksheet · {data.themeId}</span>
        <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
          <Button variant="pill" size="sm" icon={<Printer className="w-3.5 h-3.5" />} onClick={printIt}>
            Print
          </Button>

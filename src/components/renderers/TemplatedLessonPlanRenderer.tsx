...
import { Download, Maximize2, Minimize2, ExternalLink, Printer } from "lucide-react";
import toast from "react-hot-toast";
import Button from "@/components/shared/Button";
import type { TemplatedLessonPlan } from "@/lib/template-types";
import { renderLessonPlan } from "@/lib/template-engine";

export default function TemplatedLessonPlanRenderer({ data }: { data: TemplatedLessonPlan }) {
  ...
  useEffect(() => {
    ...
    renderLessonPlan(data)
      ...
      .catch((e) => {
        if (!cancelled) { toast.error("Could not render lesson plan: " + (e?.message || String(e))); setBusy(false); }
      });
  ...
  const printIt = () => { iframeRef.current?.contentWindow?.print(); };
  ...
  return (
    <div ref={wrapRef} className="bg-white dark:bg-deep-surface rounded-3xl ss-edge overflow-hidden flex flex-col h-[85vh]">
      <div className="flex items-center justify-between gap-2 px-4 py-3 border-b-2 border-ss-ink-900 dark:border-white/50 bg-soft-cream dark:bg-deep-cream/40">
        <span className="eyebrow">Lesson Plan · {data.themeId}</span>
        <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
          <Button variant="pill" size="sm" icon={<Printer className="w-3.5 h-3.5" />} onClick={printIt}>Print</Button>

...
import type { TemplatedGame } from "@/lib/template-types";
import { renderGame } from "@/lib/template-engine";

export default function TemplatedGameRenderer({ data }: { data: TemplatedGame }) {
  ...
  useEffect(() => {
    ...
    renderGame(data)
      ...
      .catch((e) => {
        if (!cancelled) { toast.error("Could not render game: " + (e?.message || String(e))); setBusy(false); }
      });
  ...
  return (
    <div ref={wrapRef} className="bg-white dark:bg-deep-surface rounded-3xl ss-edge overflow-hidden flex flex-col h-[85vh]">
      <div className="flex items-center justify-between gap-2 px-4 py-3 border-b-2 border-ss-ink-900 dark:border-white/50 bg-soft-cream dark:bg-deep-cream/40">
        <span className="eyebrow">Live game · {data.themeId} · {data.data.gameType}</span>
